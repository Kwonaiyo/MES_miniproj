...
namespace KDTB_FORMS
{
    public partial class QM_Deleted : DC00_WinForm.BaseMDIChildForm
    {
        ...
        { 
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "DELSEQ",         "폐기순번", GridColDataType_emu.VarChar,     80, HAlign.Center, false, false);
            _GridUtil.InitColumnUltraGrid(grid1, "DELDATE",        "폐기일자", GridColDataType_emu.VarChar,    100, HAlign.Center, true,  false);
            _GridUtil.InitColumnUltraGrid(grid1, "DELNO",          "폐기번호", GridColDataType_emu.VarChar,    140, HAlign.Center, true,  false);
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "WHCODE",         "창고",     GridColDataType_emu.VarChar,    160, HAlign.Left,   true,  false);
            _GridUtil.InitColumnUltraGrid(grid1, "INOUTCODE",      "입출유형", GridColDataType_emu.VarChar,    100, HAlign.Left,   true,  false);
            _GridUtil.InitColumnUltraGrid(grid1, "INOUTFLAG",      "입출구분", GridColDataType_emu.VarChar,     80, HAlign.Left,   true,  false);
            _GridUtil.InitColumnUltraGrid(grid1, "DELQTY",         "폐기수량", GridColDataType_emu.Double,      80, HAlign.Right,  true,  false);
            _GridUtil.InitColumnUltraGrid(grid1, "UNITCODE",       "단위",     GridColDataType_emu.VarChar,     70, HAlign.Left,   true,  false);
            _GridUtil.InitColumnUltraGrid(grid1, "DELWHY",         "불량원인", GridColDataType_emu.VarChar,    170, HAlign.Left,   true,  false);
            _GridUtil.InitColumnUltraGrid(grid1, "DELREASON",      "폐기사유", GridColDataType_emu.VarChar,    170, HAlign.Left,   true,  false);
            _GridUtil.InitColumnUltraGrid(grid1, "WORKER",         "작업자",   GridColDataType_emu.VarChar,     70, HAlign.Center, true,  false);
            ...
            rtnDtTemp = Common.StandardCODE("INOUTFLAG"); // 입출구분
            UltraGridUtil.SetComboUltraGrid(this.grid1, "INOUTFLAG", rtnDtTemp);

            rtnDtTemp = Common.StandardCODE("FAULT_WHY"); // 폐기 원인
            UltraGridUtil.SetComboUltraGrid(this.grid1, "DELWHY", rtnDtTemp);
        ...
        private void DoFind()
        {
            ...
            { 
                ...
                string sDelNo     = Convert.ToString(txtDelNo.Text );
                string sStartdate = string.Format("{0:yyyy-MM-dd}", dtpStart.Value);
                string sEndDate   = string.Format("{0:yyyy-MM-dd}", dtpEnd.Value);

                rtnDtTemp = helper.FillTable("TGR_QM_Deleted_S1", CommandType.StoredProcedure
                                                                 , helper.CreateParameter("PLANTCODE", sPlantCode)
                                                                 , helper.CreateParameter("ITEMCODE",  sItemCode)
                                                                 , helper.CreateParameter("DELNO",     sDelNo)
