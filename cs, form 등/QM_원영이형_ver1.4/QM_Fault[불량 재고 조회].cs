...
namespace KDTB_FORMS
{
    public partial class QM_Fault : DC00_WinForm.BaseMDIChildForm
    {
        ...
        private void btnConfirm_Click(object sender, EventArgs e)
        {
            ...
            double dtxtProdQty;
            double dtxtBadQty;
            if (String.IsNullOrEmpty(txtProdQty.Text) == true) dtxtProdQty = 0; // 자꾸 null값 오류나서 일단 이렇게함;;
            ...
            try
            {
                ...
                foreach (DataRow dr in dtTemp.Rows)
                {
                    ...
                    double iProdQty = Convert.ToDouble(txtProdQty.Text);
                    double iBadQty = Convert.ToDouble(txtBadQty.Text);
                    string sProdReason = Convert.ToString(txtProdReason.Text);
                    string sBadWhy = Convert.ToString(cboBadWhy.Text);
                    string sBadReason = Convert.ToString(txtBadReason.Text);

                    helper.ExecuteNoneQuery("TGR_QM_Fault_I1", CommandType.StoredProcedure, helper.CreateParameter("PLANTCODE",    dr["PLANTCODE"])
                                                                                          , helper.CreateParameter("FAULTNO",      dr["FAULTNO"])
                                                                                          , helper.CreateParameter("PRODQTY",      iProdQty)
                                                                                          , helper.CreateParameter("FAULTQTY",     iBadQty)
                                                                                          , helper.CreateParameter("PRODREASON",   sProdReason)
                                                                                          , helper.CreateParameter("DELETEWHY",    sBadWhy)
                                                                                          , helper.CreateParameter("DELETEREASON", sBadReason)
        ...
        #region < 양품수량(txtProdQty), 불량수량(txtBadQty) 숫자만 입력받는 메소드 >
