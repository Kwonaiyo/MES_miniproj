    ...
    public partial class QM_Fault : DC00_WinForm.BaseMDIChildForm
    {
        ...
        public void doSave()
        {
            ...
            foreach (DataRow dr in dtTemp.Rows)
            {
                ...
                dt_A.Rows.Add("0", dr["PLANTCODE"], dr["WORKCENTERCODE"], dr["FAULTNO"], sClotno);    // 가져온 CLOTNO들을 dt_A 데이터테이블에 추가
            ...
            try
            {
                ...
                if (dtxtProdQty != 0)
                {
                    ...
                    {
                        helper.Rollback();
                        ShowDialog("재검사를 중단합니다. 내용이 저장되지 않습니다.");
                        return;
                    }
                    for (int i = 0; i < sClot.Length; i++)
                    {
                        if (dtxtProdQty == 0) break;
